"""Demo alternatywne: Sparse Optical Flow (Shi-Tomasi + Lucas-Kanade).

Metoda:
- wykrywa dobre punkty (corners),
- sledzi je miedzy klatkami,
- rysuje trajektorie ruchu.

Wymagania:
    pip install opencv-python numpy

Uruchomienie:
    python demo_klt_trajectories_sign.py
"""

from __future__ import annotations

import csv
import time
from collections import deque
from dataclasses import dataclass
from pathlib import Path, PureWindowsPath
from typing import Optional

import cv2
import numpy as np


# =============================
# Konfiguracja
# =============================
CSV_PATH = Path("merged_datasets/universal_metadata_has_video_no_anomalies.csv")
VIDEO_ID = "31220"
SOURCE_FILTER: Optional[str] = "signingsavvy"
PLAY_MODE = "resolution_sweep"  # "single" albo "resolution_sweep"
WINDOW_NAME = "KLT trajectories demo"
DISPLAY_SIZE = (1200, 700)
EXTRA_SLEEP_SECONDS = 0.02
SWEEP_SOURCE_FILTER: Optional[str] = None
SWEEP_MAX_VIDEOS = 12

# Parametry detekcji i trackingu punktow
MAX_CORNERS = 180
QUALITY_LEVEL = 0.01
MIN_DISTANCE = 7
BLOCK_SIZE = 7

LK_WIN_SIZE = 31
LK_MAX_LEVEL = 4
FB_ERR_THRESHOLD = 3.2

# Parametry zachowania trackow
TRACK_LEN = 18
DETECT_INTERVAL = 2
MIN_DISPLACEMENT_TO_DRAW = 1.2
MIN_TRACKS_FOR_STABLE = 60

# Wstepna obrobka pod szybki ruch i niskokontrastowe klatki.
USE_CLAHE = True
CLAHE_CLIP_LIMIT = 2.0
CLAHE_TILE_GRID = 8
PRE_BLUR_KERNEL = 3


@dataclass
class VideoRecord:
    label: str
    source: str
    video_path: Path
    video_width: int
    video_height: int


@dataclass
class TrackPoint:
    x: float
    y: float


def normalize_video_id(value: str) -> str:
    value = str(value).strip()
    if not value:
        raise ValueError("VIDEO_ID nie moze byc pusty")
    if value.isdigit():
        return str(int(value))
    return value


def get_video_id_from_path(video_path: str) -> str:
    return normalize_video_id(PureWindowsPath(video_path).stem)


def parse_dimension(value: str) -> int:
    try:
        return int(float(value))
    except (TypeError, ValueError):
        return 0


def load_video_record(csv_path: Path, video_id: str, source_filter: Optional[str]) -> VideoRecord:
    if not csv_path.exists():
        raise FileNotFoundError(f"Nie znaleziono pliku CSV: {csv_path}")

    wanted_id = normalize_video_id(video_id)
    source_filter_lc = source_filter.lower().strip() if source_filter else None

    with csv_path.open("r", encoding="utf-8", newline="") as f:
        reader = csv.DictReader(f)
        for row in reader:
            if not row:
                continue

            row_video_path = row.get("video_path", "")
            if not row_video_path:
                continue

            if get_video_id_from_path(row_video_path) != wanted_id:
                continue

            row_source = str(row.get("source", ""))
            if source_filter_lc and row_source.lower().strip() != source_filter_lc:
                continue

            return VideoRecord(
                label=str(row.get("label", "")),
                source=row_source,
                video_path=Path(row_video_path),
                video_width=parse_dimension(row.get("video_width", "0")),
                video_height=parse_dimension(row.get("video_height", "0")),
            )

    hint = f" i zrodla '{source_filter}'" if source_filter else ""
    raise LookupError(f"Nie znaleziono rekordu dla VIDEO_ID='{video_id}'{hint}.")


def load_records_distinct_resolutions(
    csv_path: Path,
    source_filter: Optional[str],
    max_videos: int,
) -> list[VideoRecord]:
    if not csv_path.exists():
        raise FileNotFoundError(f"Nie znaleziono pliku CSV: {csv_path}")

    source_filter_lc = source_filter.lower().strip() if source_filter else None
    by_resolution: dict[tuple[int, int], VideoRecord] = {}

    with csv_path.open("r", encoding="utf-8", newline="") as f:
        reader = csv.DictReader(f)
        for row in reader:
            if not row:
                continue

            if str(row.get("has_video", "")).lower().strip() != "true":
                continue

            row_source = str(row.get("source", ""))
            if source_filter_lc and row_source.lower().strip() != source_filter_lc:
                continue

            row_video_path = row.get("video_path", "")
            if not row_video_path:
                continue

            w = parse_dimension(row.get("video_width", "0"))
            h = parse_dimension(row.get("video_height", "0"))
            if w <= 0 or h <= 0:
                continue

            key = (w, h)
            if key in by_resolution:
                continue

            by_resolution[key] = VideoRecord(
                label=str(row.get("label", "")),
                source=row_source,
                video_path=Path(row_video_path),
                video_width=w,
                video_height=h,
            )

    records = sorted(by_resolution.values(), key=lambda r: (r.video_width * r.video_height, r.video_width, r.video_height))
    if max_videos > 0:
        records = records[:max_videos]
    return records


def _detect_points(gray: np.ndarray) -> Optional[np.ndarray]:
    return cv2.goodFeaturesToTrack(
        gray,
        maxCorners=MAX_CORNERS,
        qualityLevel=QUALITY_LEVEL,
        minDistance=MIN_DISTANCE,
        blockSize=BLOCK_SIZE,
    )


def _preprocess_gray(gray: np.ndarray) -> np.ndarray:
    out = gray
    if USE_CLAHE:
        clahe = cv2.createCLAHE(clipLimit=CLAHE_CLIP_LIMIT, tileGridSize=(CLAHE_TILE_GRID, CLAHE_TILE_GRID))
        out = clahe.apply(out)
    if PRE_BLUR_KERNEL > 1:
        out = cv2.GaussianBlur(out, (PRE_BLUR_KERNEL, PRE_BLUR_KERNEL), 0)
    return out


def _init_tracks(points: Optional[np.ndarray]) -> list[deque[TrackPoint]]:
    tracks: list[deque[TrackPoint]] = []
    if points is None:
        return tracks

    for p in points.reshape(-1, 2):
        dq: deque[TrackPoint] = deque(maxlen=TRACK_LEN)
        dq.append(TrackPoint(float(p[0]), float(p[1])))
        tracks.append(dq)
    return tracks


def _draw_tracks(canvas: np.ndarray, tracks: list[deque[TrackPoint]]) -> tuple[int, float]:
    if not tracks:
        return 0, 0.0

    total_disp = 0.0
    valid_tracks = 0

    for tr in tracks:
        if len(tr) < 2:
            continue

        p0 = tr[0]
        p1 = tr[-1]
        disp = float(np.hypot(p1.x - p0.x, p1.y - p0.y))
        total_disp += disp
        valid_tracks += 1

        if disp < MIN_DISPLACEMENT_TO_DRAW:
            continue

        pts = np.array([[int(pt.x), int(pt.y)] for pt in tr], dtype=np.int32)
        cv2.polylines(canvas, [pts], False, (0, 255, 255), 2, cv2.LINE_AA)
        cv2.circle(canvas, (int(p1.x), int(p1.y)), 2, (0, 0, 255), -1, cv2.LINE_AA)

    mean_disp = total_disp / valid_tracks if valid_tracks > 0 else 0.0
    return valid_tracks, mean_disp


def run_demo(record: VideoRecord, current_index: Optional[int] = None, total_count: Optional[int] = None) -> bool:
    if not record.video_path.exists():
        raise FileNotFoundError(
            f"Plik wideo nie istnieje: {record.video_path}\n"
            "Sprawdz, czy sciezki w CSV sa poprawne na tym komputerze."
        )

    cap = cv2.VideoCapture(str(record.video_path))
    if not cap.isOpened():
        raise RuntimeError(f"Nie udalo sie otworzyc wideo: {record.video_path}")

    fps = cap.get(cv2.CAP_PROP_FPS)
    fps = fps if fps and fps > 0 else 30.0
    frame_interval = 1.0 / fps

    ok, first_frame = cap.read()
    if not ok:
        cap.release()
        raise RuntimeError("Nie mozna odczytac pierwszej klatki")

    prev_gray_raw = cv2.cvtColor(first_frame, cv2.COLOR_BGR2GRAY)
    prev_gray = _preprocess_gray(prev_gray_raw)
    initial_points = _detect_points(prev_gray)
    tracks = _init_tracks(initial_points)

    lk_params = dict(
        winSize=(LK_WIN_SIZE, LK_WIN_SIZE),
        maxLevel=LK_MAX_LEVEL,
        criteria=(cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 20, 0.03),
    )

    frame_idx = 0

    cv2.namedWindow(WINDOW_NAME, cv2.WINDOW_NORMAL)
    cv2.resizeWindow(WINDOW_NAME, DISPLAY_SIZE[0], DISPLAY_SIZE[1])

    while True:
        loop_start = time.perf_counter()

        ok, frame = cap.read()
        if not ok:
            break
        frame_idx += 1

        gray_raw = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        gray = _preprocess_gray(gray_raw)
        display = frame.copy()

        if tracks:
            p0 = np.float32([[tr[-1].x, tr[-1].y] for tr in tracks]).reshape(-1, 1, 2)
            p1, st, _err = cv2.calcOpticalFlowPyrLK(prev_gray, gray, p0, None, **lk_params)
            p0r, st_back, _err_back = cv2.calcOpticalFlowPyrLK(gray, prev_gray, p1, None, **lk_params)

            if p1 is not None and st is not None and p0r is not None and st_back is not None:
                good = (st.reshape(-1) == 1) & (st_back.reshape(-1) == 1)
                fb_err = np.linalg.norm((p0.reshape(-1, 2) - p0r.reshape(-1, 2)), axis=1)
                good &= fb_err < FB_ERR_THRESHOLD

                new_tracks: list[deque[TrackPoint]] = []
                for tr, ok_flag, pt in zip(tracks, good, p1.reshape(-1, 2)):
                    if not ok_flag:
                        continue
                    x, y = float(pt[0]), float(pt[1])
                    if not (0 <= x < gray.shape[1] and 0 <= y < gray.shape[0]):
                        continue
                    tr.append(TrackPoint(x, y))
                    new_tracks.append(tr)
                tracks = new_tracks

        need_more_points = len(tracks) < MIN_TRACKS_FOR_STABLE
        if frame_idx % DETECT_INTERVAL == 0 or need_more_points:
            mask = np.full_like(gray, 255)
            for tr in tracks:
                x, y = int(tr[-1].x), int(tr[-1].y)
                cv2.circle(mask, (x, y), MIN_DISTANCE + 2, 0, -1)

            p_new = cv2.goodFeaturesToTrack(
                gray,
                maxCorners=max(0, MAX_CORNERS - len(tracks)),
                qualityLevel=QUALITY_LEVEL,
                minDistance=MIN_DISTANCE,
                blockSize=BLOCK_SIZE,
                mask=mask,
            )
            if p_new is not None:
                for p in p_new.reshape(-1, 2):
                    dq: deque[TrackPoint] = deque(maxlen=TRACK_LEN)
                    dq.append(TrackPoint(float(p[0]), float(p[1])))
                    tracks.append(dq)

        track_count, mean_disp = _draw_tracks(display, tracks)

        order = ""
        if current_index is not None and total_count is not None:
            order = f"[{current_index}/{total_count}] "

        title = f"{order}{record.video_path.name} | label={record.label} | src={record.source}"
        stats = (
            f"tracks={track_count} | mean_disp={mean_disp:.2f}px | "
            f"win={LK_WIN_SIZE} lvl={LK_MAX_LEVEL} fb<{FB_ERR_THRESHOLD:.1f}"
        )
        hint = "q/ESC = wyjscie, n = nastepne"

        cv2.putText(display, title, (12, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (0, 255, 0), 2)
        cv2.putText(display, stats, (12, 56), cv2.FONT_HERSHEY_SIMPLEX, 0.56, (255, 255, 255), 1)
        cv2.putText(display, hint, (12, 84), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1)

        display = cv2.resize(display, DISPLAY_SIZE, interpolation=cv2.INTER_AREA)
        cv2.imshow(WINDOW_NAME, display)

        key = cv2.waitKey(1) & 0xFF
        if key in (27, ord("q")):
            cap.release()
            cv2.destroyAllWindows()
            return True
        if key == ord("n"):
            break

        prev_gray = gray

        elapsed = time.perf_counter() - loop_start
        sleep_time = max(0.0, frame_interval - elapsed) + max(0.0, EXTRA_SLEEP_SECONDS)
        if sleep_time > 0:
            time.sleep(sleep_time)

    cap.release()
    cv2.destroyAllWindows()
    return False


def run_resolution_sweep() -> None:
    records = load_records_distinct_resolutions(CSV_PATH, SWEEP_SOURCE_FILTER, SWEEP_MAX_VIDEOS)
    if not records:
        raise LookupError("Nie znaleziono rekordow do trybu resolution_sweep.")

    print(f"Tryb resolution_sweep: {len(records)} nagran")
    for i, rec in enumerate(records, start=1):
        print(f"[{i}/{len(records)}] {rec.video_path.name} | {rec.video_width}x{rec.video_height} | {rec.label} | {rec.source}")
        should_quit = run_demo(rec, i, len(records))
        if should_quit:
            break


def main() -> None:
    if PLAY_MODE == "single":
        record = load_video_record(CSV_PATH, VIDEO_ID, SOURCE_FILTER)
        print(f"Uruchamiam KLT demo dla: {record.video_path}")
        print(f"label={record.label}, source={record.source}, src_res={record.video_width}x{record.video_height}")
        run_demo(record)
        return

    if PLAY_MODE == "resolution_sweep":
        run_resolution_sweep()
        return

    raise ValueError("PLAY_MODE musi byc 'single' albo 'resolution_sweep'.")


if __name__ == "__main__":
    main()
